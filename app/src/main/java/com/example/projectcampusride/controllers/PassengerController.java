...
public class PassengerController {
    ...
    public void getPassengerRides(String passengerId, PassengerController.PassengerRidesCallback callback) {
        passengerRepository.getPassengerRides(passengerId)
                .addOnSuccessListener(querySnapshot -> {
                    if (querySnapshot != null && !querySnapshot.isEmpty()) {
                        List<Ride> rides = new ArrayList<>();
                        // גישה לכל מסמך ב-querySnapshot
                        for (Ride document : querySnapshot) {
                            try {
                                Ride ride = document;
                                if (ride != null) {
                                    rides.add(ride);
                                ...
                                Log.e("PassengerRides", "Error parsing ride: " + e.getMessage(), e);
                            }
                        }
                        if (!rides.isEmpty()) {
                            callback.onSuccess(rides);
                        } else {
                            callback.onFailure("No rides found for the passenger.");
                        }
                    } else {
                        callback.onFailure("No requests found for the passenger.");
                    }
                })
                .addOnFailureListener(e -> {
    ...
    public interface PassengerRidesCallback {
        void onSuccess(List<Ride> rides);
        void onFailure(String errorMessage);
    }
}
